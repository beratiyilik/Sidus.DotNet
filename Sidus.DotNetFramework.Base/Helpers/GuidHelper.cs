...
namespace System
{
    ...
    public class DateTimePrecise
    {
        ...
        public DateTimeOffset GetUtcNow()
        {
            ...
            if (Math.Abs((preciseNow - now).TotalSeconds) > _divergentSeconds) return now;
    ...
    public static class TimestampHelper
    {
        ...
        public static DateTimeOffset FromCassandraTimestamp(long ts)
        {
            ...
            if (ts <= MaxUnixSeconds) ts *= TicksInOneSecond;

            // convert a timestamp in milliseconds to ticks
            if (ts <= MaxUnixMilliseconds) ts *= TicksInOneMillisecond;

            // convert a timestamp in microseconds to ticks
            if (ts <= MaxUnixMicroseconds) ts *= TicksInOneMicrosecond;
    ...
    public static class GuidGenerator
    {
        ...
        private
        const int VariantByteMask = 0x3f;
        private
        const int VariantByteShift = 0x80;
        ...
        public static GuidGeneration GuidGeneration
        {
            get;
            set;
        }

        public static byte[] NodeBytes
        {
            get;
            set;
        }
        public static byte[] ClockSequenceBytes
        {
            get;
            set;
        }
        ...
        public static byte[] GenerateNodeBytes(IPAddress ip)
        {
            ...
            if (bytes.Length < 6) throw new ArgumentOutOfRangeException("ip", "The passed in IP address must contain at least 6 bytes.");
        ...
        {
            if (mac == null) throw new ArgumentNullException("mac");
        ...
        public static byte[] GenerateClockSequenceBytes(long ticks)
        {
            ...
            if (bytes.Length == 1) return new byte[] {
                0x0,
                bytes[0]
            };
            ...
            switch (GuidGeneration)
            {
                ...
                default:
                    lock (Lock)
                    {
                        ...
                        if (ts <= _lastTimestampForNoDuplicatesGeneration) ClockSequenceBytes = GenerateClockSequenceBytes();
        ...
        {
            if (clockSequence == null) throw new ArgumentNullException("clockSequence");

            if (node == null) throw new ArgumentNullException("node");

            if (clockSequence.Length != 2) throw new ArgumentOutOfRangeException("clockSequence", "The clockSequence must be 2 bytes.");
